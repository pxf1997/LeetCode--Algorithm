...
//        int maxIceCream = solution.maxIceCream(new int[]{1, 3, 2, 4, 1}, 7);
        int maxIceCream = solution.maxIceCream(new int[]{1, 6, 3, 1, 2, 5}, 20);
        System.out.println("maxIceCream = " + maxIceCream);
    }

    //力扣代码
    //leetcode submit region begin(Prohibit modification and deletion)
    // 分析--是不是贪心? 策略--优先买价格低的是否正确
    // 数学证明贪心是正确的,而且事实上没法用dp做!!!
    class Solution {
        public int maxIceCream(int[] costs, int coins) {
            Arrays.sort(costs);
            int cnt = 0;
            while (cnt < costs.length && coins >= costs[cnt]) { // 短路与,先判断前面
                coins -= costs[cnt];
                cnt++;
            }
            return cnt;
